...
def get_hashcat_string(mpinfos: MasterPasswordInfos) -> str:
    """
    Print the hashcat format string.
    ...
    if mpinfos.no_master_password:
        ...
        if mpinfos.mode == '3des':
            ...
            s += f'AES*{hex(mpinfos.global_salt)}*{hex(mpinfos.entry_salt)}*{mpinfos.iteration}*{hex(mpinfos.iv)}*{hex(mpinfos.cipher_text)}'

        return s


def main():
    parser = argparse.ArgumentParser(description="Extract hash from a Firefox password database (key3.db, key4.db).")
    parser.add_argument("dbfile", help="Path to the Firefox key3.db or key4.db file.")
    args = parser.parse_args()

    db_filepath = args.dbfile
    db_version = 3 if db_filepath.endswith("key3.db") else 4

    print(f"Extracting from {db_filepath}, version {db_version}")
    mp_infos = extract_master_password_infos(db_filepath, db_version)

    print(get_hashcat_string(mp_infos))
